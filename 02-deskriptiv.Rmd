...
Oszlopdiagramot használtunk a következő ábrán (\@ref(fig:oszlopdiagram). ábra).
...
data.frame( Ci0 = seq( 500, 4500, 500 ), Ci1 = seq( 1000, 5000, 500 ), fi = tab, gi = prop.table( tab ) )
```
...
A születési tömegek medián abszolút eltérése `r mad( birthwt$bwt )` gramm, tehát az újszülöttek testtömegeinek mediánjuk körül vett (abszolút) ingadozásának mediánja `r mad( birthwt$bwt )` gramm. (Pontosabban szólva ezek az értékek a fent definiált $MAD$ 1,4826-szorosai, a konstans szorzó értelme, hogy így a normális eloszlás $MAD$-ja épp a szórás lesz.)
...
A legnagyobb kihívás azonban az osztályközök helyes megválasztása. Ez a probléma teljesen ugyanaz, mint amit az osztályközös gyakorisági sornál is kifejtettünk. Sőt, itt talán még jobban szemléltethető: a következő ábrán ugyanazt az adatsort ábrázoltuk, csak épp az optimálisnál lényegesen több, illetve lényegesen kevesebb osztályközt használva is (\@ref(fig:hisztogramvalasztasok). ábra).
...
Mindezeket szemlélteti a következő ábra is (\@ref(fig:corrdemo). ábra), mely különböző korrelációs együtthatójú kapcsolatokat (különböző előjelekkel és abszolút értékekkel, azaz különböző irányú és erősségű kapcsolatokat) mutat be példákkal.
